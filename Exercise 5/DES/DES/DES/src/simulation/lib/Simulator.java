...
public class Simulator implements IEventObserver{
	...
	private void updateStatsSCE(Customer currentCustomer){
        ...
        if(this.state.isTransientPhaseOver()) {
            if (currentCustomer != null) {
				 ...
				 */
            	DiscreteConfidenceCounterWithRelativeError ccreSingleWaitingTime = (DiscreteConfidenceCounterWithRelativeError) sims.statisticObjects.get(sims.ccreWaitingTime);
            	ccreSingleWaitingTime.count(simTimeToRealTime(currentCustomer.getTimeInQueue()));

            	DiscreteCounter ccDummyWaitingTime = (DiscreteCounter) sims.statisticObjects.get(sims.ccDummyWaitingTime);
				ccDummyWaitingTime.count(simTimeToRealTime(currentCustomer.getTimeInQueue()));

				DiscreteCounter tempdtcBatchWaitingTimeTemp = (DiscreteCounter) sims.statisticObjects.get(sims.tempdtcBatchWaitingTime);
				tempdtcBatchWaitingTimeTemp.count(simTimeToRealTime(currentCustomer.getTimeInQueue()));

				DiscreteCounter tempdtcBatchServiceTimeTemp = (DiscreteCounter) sims.statisticObjects.get(sims.tempdtcBatchServiceTime);
				tempdtcBatchServiceTimeTemp.count(simTimeToRealTime(currentCustomer.getTimeInService()));

				if(this.state.numSamplesInCurrentBatch == sims.batchLength){

					DiscreteConfidenceCounterWithRelativeError ccreWaitingTimeBatchTemp = (DiscreteConfidenceCounterWithRelativeError) sims.statisticObjects.get(sims.ccreBatchWaitingTime);
					double ccreWaitingTimeTempMean = ccDummyWaitingTime.getMean();
					ccreWaitingTimeBatchTemp.count(ccreWaitingTimeTempMean);
					ccDummyWaitingTime.reset();

					DiscreteConfidenceCounterWithRelativeError dtcBatchWaitingTimeTemp = (DiscreteConfidenceCounterWithRelativeError) sims.statisticObjects.get(sims.dtcBatchWaitingTime);
					double dtcWaitingTimeTempMean = tempdtcBatchWaitingTimeTemp.getMean();
					dtcBatchWaitingTimeTemp.count(dtcWaitingTimeTempMean);

					DiscreteAutocorrelationCounter dtacBatchWaitingTimeTemp = (DiscreteAutocorrelationCounter) sims.statisticObjects.get(sims.dtacBatchWaitingTime);
					dtacBatchWaitingTimeTemp.count(dtcWaitingTimeTempMean);

					tempdtcBatchWaitingTimeTemp.reset();

					DiscreteConfidenceCounterWithRelativeError dtcBatchServiceTimeTemp = (DiscreteConfidenceCounterWithRelativeError) sims.statisticObjects.get(sims.dtcBatchServiceTime);
					double dtcServiceTimeTempMean = tempdtcBatchServiceTimeTemp.getMean();
					dtcBatchServiceTimeTemp.count(dtcServiceTimeTempMean);
					tempdtcBatchServiceTimeTemp.reset();

					if (dtcBatchWaitingTimeTemp.getMean() >= 5 * dtcBatchServiceTimeTemp.getMean()){
						sims.numBatchWaitingTimeExceeds5TimesBatchServiceTime += 1;
					}

					if (ccreWaitingTimeBatchTemp.maxRelErr() < 0.05 || ccreWaitingTimeBatchTemp.maxAbsErr() < 0.0001){
						pushNewEvent(new SimulationTerminationEvent(getSimTime()+1));
						System.out.println("pushed termination event");
					}
                ...
                double meanServiceTime = ((DiscreteCounter)sims.statisticObjects.get(sims.dtcServiceTime)).getMean();
                if (currentCustomer.getTimeInQueue() >= 5 * meanServiceTime){
                	sims.numWaitingTimeExceeds5TimesServiceTime += 1;
				}
	...
	public long getStateNumberOfSamples(){
		return this.state.numSamples;
	}
	public long getStateNumberOfBatches(){
		return this.state.numBatches;
	}
}
